...
class TAggregate(nn.Module):
  ...
  def forward(self, x, mask):
    ...
    x.transpose_(1, 0)
    ones_tensor = torch.ones(mask.shape[0], 1).float().to(mask.device)
    pad_mask = torch.cat((ones_tensor, mask), axis=1) == 0
    o = self.transformer_enc(x, src_key_padding_mask=pad_mask)
